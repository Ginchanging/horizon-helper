...
GameSave Guardian is a portable Windows tool for backing up Xbox game saves, optionally recovering focus for a selected window, running AFK key loops, and launching a few game automation workflows.
...
6. Use the Automation tab for AutoBuyCar or FindNewSubaru workflows.
...
- `StartAutomation.cmd`: start `AutoBuyCar` or `FindNewSubaru`. Examples: `.\StartAutomation.ps1 -Mode AutoBuyCar -LoopCount 3`, `.\StartAutomation.ps1 -Mode FindNewSubaru -LoopCount 1`.
...
Logs are written to `logs\backup.log`, `logs\focus-lock.log`, `logs\afk.log`, and `logs\automation.log`. Runtime state is stored in `runtime\`.
...
The package is written to `dist\gamesave-guardian-v1.4.0.zip`. It includes the GUI, scripts, config, and docs, and excludes backups, logs, runtime files, `.git`, and old zip files.
