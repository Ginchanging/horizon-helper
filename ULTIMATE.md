...
- **估计完成时间(ETA):**
  ...
  - **内圈阶段进度**:三个内圈循环(`Sequence`/`AutoBuyCar`/`FindNewSubaru`)在**每次迭代开头**额外往同一个 `ultimate-progress.json` 写 `phase`/`phaseCurrent`/`phaseTotal`(Sequence、AutoBuyCar 直接写;FindNewSubaru 经 `Invoke-AutomationFindNewSubaruLoop` 的可选 `-ProgressCallback` 回调写,Automation 子系统不传该回调,行为不变)。GUI「Ultimate」页据此实时显示**当前跑到第几次 Sequence / AutoBuyCar / FindNewSubaru** + 一条阶段进度条。轮/暂停/完成等边界写入不带 phase(= 当前无内圈阶段),字段缺省即清空。执行步骤本身未变,只是多了进度上报。
...
跑到一半想去用电脑、又不想 Stop 后从头来,可以**暂停**,过会儿再**继续**。

- **入口:** GUI「Ultimate」标签页的 **Pause / Resume** 切换按钮(仅 Ultimate 有;只在运行中可点)。无命令行脚本。
- **机制(文件标志):** worker 是 `Start-Process -WindowStyle Hidden` 起的独立进程,GUI 与它只能靠文件通信。点 Pause → GUI 写 `runtime/ultimate.pause`(`Set-UltimatePause`);点 Resume → 删该文件(`Clear-UltimatePause`)。worker 用 `Test-UltimatePause` 在安全边界轮询。
- **只在「安全边界」暂停(非即时):** 按键自动化不能停在「一次键程中间」(会让游戏菜单/比赛错位),所以 worker 只在这 4 个离散边界检查标志(`Wait-UltimatePauseGate`):
  1. 每个**整轮 workflow** 开始前(下一轮 Prelude 之前);
  2. **Sequence 每一小轮**开始前(发起跑 Enter 之前)—— 覆盖那 1.5 小时的主体,最关键;
  3. **AutoBuyCar 每一轮**之间;
  4. **FindNewSubaru 每一轮**之间(经 `Invoke-AutomationFindNewSubaruLoop` 的可选 `-PauseCheck` 回调)。
  - 后果:点 Pause 后**最多等当前这一小步走完**(Sequence 一小轮 ≈ 50s)才真正停住。
- **暂停时的行为:** 松开 `W`(`Release-AfkKeys`)、油门归零(`Set-AfkGamepadRightTrigger 0`),原地 `while (Test-UltimatePause) { sleep 500ms }` 空转;进度写 `status='paused'`,GUI 状态栏前缀 `PAUSED`(红色)。此时不再发任何按键,可自由 alt-tab。
- **继续时:** 复用 `startupDelaySeconds`(默认 5s)倒计时,**给你切回游戏窗口的时间**,再从原处继续发键。
- **清理:** worker 启动时(`Set-UltimatePid` 之后)先清一次陈旧标志,免得上次遗留的标志一开跑就暂停;worker `finally` 退出时也清。被强制 Stop 时,GUI 的 `Stop-AppUltimate` / 下次启动会清掉它。
- **DryRun:** 闸门直接放行,不阻塞(dry-run 测试不会卡住)。
...
> **卡死兜底(每小轮两次 X 之后)**:车子有时开到 99% 被卡在墙边到不了终点,两次 X **不会**弹出「重新开始赛事」确认框,后面的 Enter 落空、整条流程卡死。兜底逻辑(`Test-UltimateSequenceRestartDialog` / `Invoke-UltimateSequenceStuckRecovery`,`RunUltimate.ps1`):
> - **检测**:两次 X 后等 ~600ms,截前台窗口中央区域 → zh-Hans WinRT OCR(复用 `Invoke-AutomationOcrImagePath`)找关键字「重新开始」。**最多重试 3 次、任一次读到即判「已出现」**;全读不到才判「未出现」,以免误判把恢复宏砸进正常对话框。仅当**检测到对话框**时才按那最后一个确认 Enter(选「嗯」重开)。
> - **恢复(未出现时)**:先 `Set-AfkGamepadRightTrigger -Value 0` 松开 RT,再走恢复宏 `Get-DefaultUltimateSequenceRecoverySteps` = **Esc(1s) → A(1s) → Enter(1s) → Enter(10s) → Enter(5s)**,把菜单退出再重新进赛事。
> - **重做策略**:**保留已完成的小轮数**,只把**当前这一轮从它第一次按键(Enter)原地重做**(循环计数器不前进、也不回到第 1 轮),并把本阶段总轮数 **+`extraLoopsPerRecovery`(默认 2)**。
> - **上限**:连续恢复超过 `maxRecoveries`(默认 3)次仍不出对话框 → **WARN 软失败退出 Sequence 阶段**,交给外层 workflow 循环下一轮的 Prelude 重新归位(同 FindNewSubaru 的软失败思路),不让整条 Ultimate 卡死。
> - 开关/调整:`config.json` → `ultimate.sequenceStuckRecovery`(`enabled` 默认 `true`、`extraLoopsPerRecovery` 默认 `2`、`maxRecoveries` 默认 `3`)。`enabled=false` 完全关闭检测,退回原有「按完两次 X 直接按 Enter」的行为(OCR 万一误判时的逃生开关)。
> - 调试:worker 加 `-AssumeSequenceStuck` 开关(仿 `-AssumeTargetFound`)可在 `-DryRun` 下强制「未出现」,离线演练恢复→重做→+2→上限软失败全链路。
...
- **这是 CV 阶段**:需要真实桌面 + 游戏窗口;headless / 无游戏时会一直找不到。
- **脱格自愈(SoftFail,2026-06-10)**:Ultimate 调用本函数时传 `-SoftFailOnExhaust`。当某次按键被游戏多读/漏读、光标被顶出车格(绿色高亮变成 ~88×88 小方块、OCR 读空)时,有两道兜底:① 连续 `6` 次检测到「高亮不像车卡」(`Rect.Width<180` 且 `Height<150`,或根本没高亮)即判定 desync,提前结束本阶段;② 或搜索按满 `maxSearchAttempts` 仍无果,也只写 `WARN` 提前结束——**两种情况都不再 `throw`/`exit 1`**。控制权交回外层 workflow 循环,由下一轮第 5 步 Prelude(Esc×4 复位)把菜单归位后继续刷;无限/多轮运行因此能自愈,单轮也以 WARN 干净收尾。独立 Automation 子系统不传该开关,仍在耗尽后 `throw`(行为不变)。根因是 `SendKeys` 偶发多发/漏发方向键;曾尝试改用 `SendInputScanCode` 规避,但 **Forza 完全无视 `SendInput` 注入**(游戏检测不到任何按键,2026-06-10 实测后已回退 `SendKeys`),所以本自愈机制就是主要防线。代码:`Invoke-AutomationFindNewSubaruLoop` 的 `$SoftFailOnExhaust` / `$offGridStreak`(`AutomationLib.ps1`)。
...
| `inputMethod` | `SendKeys` | 按键后端。**必须用 `SendKeys`**:Forza 完全无视 `SendInput` 注入的键盘(`SendInputScanCode`/`SendInputVirtualKey` 游戏检测不到,2026-06-10 实测,与"开车时忽略 SendInput"是同一机制) |
...
| `sequenceStuckRecovery.enabled` | `true` | 第 10 步每小轮两次 X 后是否 OCR 检测「重新开始赛事」对话框并兜底恢复 |
| `sequenceStuckRecovery.extraLoopsPerRecovery` | 2 | 每次触发恢复后本阶段总轮数增加的圈数 |
| `sequenceStuckRecovery.maxRecoveries` | 3 | 连续恢复多少次仍卡死就软失败退出 Sequence 阶段 |
...
## 改动须知
- 五个宏写死在 `UltimateLib.ps1`:`Get-DefaultUltimatePreludeSteps` / `Get-DefaultUltimateAfterCodeSteps` / `Get-DefaultUltimatePostSequenceSteps` / `Get-DefaultUltimatePostBuySteps` / `Get-DefaultUltimateSequenceRecoverySteps`(第 10 步卡死兜底恢复宏:Esc,A,Enter,Enter,Enter)。改宏改这里,**并更新本文档对应表格**。
